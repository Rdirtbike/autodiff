...
import Prelude hiding (drop, replicate, sum, take, (++))
...
instance (Mode m, Vector v a, Num a) => MVector (DMVec v) (D s m a) where
  ...
  basicUnsafeReplicate n (MkD x x') = MkMV 0 <$> basicUnsafeReplicate n x <*> newSTRef (dmap (replicate n) (sum . take n) x')
  ...
  basicUnsafeGrow (MkMV o v vr) by = do
    ...
    v' <- readSTRef vr
    let n = basicLength v
    wr <- newSTRef $ dmap ((++ replicate by 0) . unsafeSlice o n) ((replicate o 0 ++) . take n) v'
    pure $ MkMV 0 w wr
